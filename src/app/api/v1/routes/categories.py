...
from src.core.category.exceptions import CategoryAlreadyExists, CategoryInUseError, CategoryNotFound
from src.domain.category import Category
from src.schemas.category import CreateCategorySchema, CategoryResponseSchema, UpdateCategorySchema
...
def get_category_by_id(
    category_id: int,
    use_case: Annotated[GetCategoryByIdUseCase, Depends(category_container.get_category_by_id)],
):
    try:
...
def update_category(
    ...
    use_case: Annotated[UpdateCategoryUseCase, Depends(category_container.update_category)],
):
    try:
        return use_case.execute(category_id, category_data.model_dump(exclude_unset=True))
...
def delete_category(
    category_id: int,
    use_case: Annotated[DeleteCategoryUseCase, Depends(category_container.delete_category)],
):
    try:
